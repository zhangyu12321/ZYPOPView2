...
def main(argvs):
    ...
    try:
        # cwd = os.getcwd()
        cwd = os.path.abspath(__file__).split('/agents.py')[0]
    ...
    for key in agentsDic.keys():
        ...
        # 4.打包
        print '3.开始打包%s(大约需要2-5分钟)'%key.encode('utf8')
        ...
        if status:
            print '4.打包%s失败'%key.encode('utf8')
        else:
            print '4.打包%s成功'%key.encode('utf8')
